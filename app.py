...
def call_gemini_with_backoff(
    ...
    raise Exception(
        "RESOURCE_EXHAUSTED: Google AI Free Tier Quota အားလုံး သို့မဟုတ် ဆာဗာချိတ်ဆက်မှု အပြည့်အဝ ကုန်ဆုံးသွားပါပြီ။"
    )
...
def scan_ocr():
    ...
    # အင်တာနက် မရှိပါက Local OCR သို့ တိုက်ရိုက်သွားမည်
    if not is_connected():
        try:
        ...
        # လိုင်းရှိပါက Gemini OCR စမ်းသုံးမည်၊ မရပါက Local OCR သို့ Automatic Fallback လုပ်မည်
        try:
            ...
            # Gemini Limit ပြည့်ခြင်း သို့မဟုတ် ချိတ်ဆက်မှု လဲကျပါက Local OCR ဖြင့် ပြန်လည် ထုတ်ပေးမည်
            print(f"⚠️ Gemini OCR Error ({str(e)}). Local Tesseract OCR သို့ ပြောင်းလဲဆောင်ရွက်နေပါသည်...")
            try:
                processed_img = preprocess_image_for_tesseract(image_bytes)
                custom_config = r"--oem 3 --psm 4"
                raw_text = pytesseract.image_to_string(
...
def ai_process():
    ...
    if not is_connected():
        return (
            jsonify({
                "success": False,
                "error": "⚠️ အင်တာနက်လိုင်းမရှိပါ သို့မဟုတ် Connection ပြတ်တောက်နေပါသည်။ AI ဝန်ဆောင်မှု အသုံးပြုရန် အင်တာနက် လိုအပ်ပါသည်။",
            }),
            400,
        )
    ...
    try:
        ...
        err_str = str(e)
        if any(x in err_str for x in ["RESOURCE_EXHAUSTED", "Quota", "429"]):
            return (
                jsonify({
                    "success": False,
                    "error": "⚠️ Gemini AI Free Quota / Usage Limit ပြည့်သွားပါပြီ။ ခေတ္တစောင့်ဆိုင်းပြီးမှ ပြန်လည်ကြိုးစားပါ။",
                }),
                429,
            )
        return (
            jsonify({"success": False, "error": f"Gemini AI Error: {err_str}"}),
            500,
        )
